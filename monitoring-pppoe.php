...
class routeros
{
     ...
     * @param string      $user      The RouterOS username
     * @param string      $pass   The RouterOS password
     *
    ...
    public function connect($host, $port, $user, $pass)
    {
        for ($ATTEMPT = 1; $ATTEMPT <= $this->attempts; $ATTEMPT++) {
            ...
            $this->debug('Connection attempt #' . $ATTEMPT . ' to ' . $PROTOCOL . $host . ':' . $port . '...');
            $this->socket = @stream_socket_client($PROTOCOL . $host . ':' . $port, $this->error_no, $this->error_str, $this->timeout, STREAM_CLIENT_CONNECT, $context);
            if ($this->socket) {
                        ...
                        if (!isset($RESPONSE[1])) {
                            ...
                            if (preg_match_all('/[^=]+/i', $RESPONSE[1], $MATCHES)) {
                                if ($MATCHES[0][0] == 'ret' && strlen($MATCHES[0][1]) == 32) {
                                    $this->write('/login', false);
                                    $this->write('=name=' . $user, false);
                                    $this->write('=response=00' . md5(chr(0) . $pass . pack('H*', $MATCHES[0][1])));
                                    $RESPONSE = $this->read(false);
...
?>
<?php
    $koneksi = mysqli_connect("localhost","naux3337_grizylnet","naux3337_grizylnet","naux3337_grizylnet");
    if (mysqli_connect_errno()){
    	echo "Koneksi database gagal : " . mysqli_connect_error();
    }
    $query = mysqli_query($koneksi,"select * from router where description='Aktif'");
    ...
    $API = new routeros();
    $API->debug = false;
	if($API->connect($host, $port, $user, $pass)){
