						...
						<center>
							...
							<div class="row mt-2">
								<div class="col-lg-6 mb-2">
									<select style="color: grey;" name="month" id="month" class="form-control mr-sm-2" required>
								...
								<div class="col-lg-4">
									<button class="btn btn-danger my-2 my-sm-0 form-control" type="submit" onclick="cek_kuota()">CEK PEMAKAIAN KUOTA</button>
								</div>
							</div>
					</div>
		...
		if (no == '') {
			...
			if (m == '') {
				...
				if (y == '') {
					...
					$.ajax({
						type: 'POST',
						data: "cek_kuota=" + 1 + "&no_services=" + no_services.val() + "&month=" + month.val() + "&year=" + year.val(),
						url: '<?= site_url('front/view_kuota') ?>',
						cache: false,
						beforeSend: function() {
