...
class Tripay extends CI_Controller
{
	...
	*/
	...
	{
		$paymentsaya = $this->db->get('payment')->result();
		...
		if ($callbackSignature != $signature) {
			...
			$myinvoice    = $this->db->get_where('invoice', array('merchant_ref' => $uniqueRef))->row_array();
			$invoice_id   = $myinvoice['invoice_id'];
			$merchantRef  = $myinvoice['merchant_ref'];
			...
			if ($event == 'payment_status') {
				if ($data->status == 'UNPAID') {
					...
					$no_invoice   = $myinvoice['invoice'];
					$bulannya     = $myinvoice['month'];
					$tahunnya     = $myinvoice['year'];
					$kodeRefrensi = $myinvoice['reference_code'];
					...
					$this->db->set('merchant_ref', $merchantRef);
					$this->db->set('status', 'GAGAL BAYAR');
					$this->db->where('invoice_id', $invoice_id);
					$this->db->update('invoice');
